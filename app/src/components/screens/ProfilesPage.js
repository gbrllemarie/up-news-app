...
class ProfilesPage extends Component {
    ...
    listProfiles() {
        return global.storage.profiles.map((profile)=> {
            return(
                <Card
                    key={profile.guid}
                    image={this.getThumbnail(profile)}>
                    <TouchableOpacity onPress={()=>this.props.navigation.navigate('ArticleView', profile)}>
                        <Text style={global.styles.articleTitle}>
                        {profile.title.toString()}
                        </Text>

                        <Text style={global.styles.datecon}>
                            {moment(profile.pubDate.slice(0,10)).format('DD MMMM YYYY')}
                        </Text>
                    </TouchableOpacity>
